...
public class Project3 {
    ...
    static final int MAX_TURN_RATE = (int)(ROBOT_RADIUS * 1.75);
    static final int MIN_TURN_RATE = 2000; //The maximum turning radius allowed by the robot
    
    static final int TARGET_DISTANCE = 16_500;
    ...
    public static void main(String[] args) throws InterruptedException, IOException {
        ...
        Runtime.getRuntime().addShutdownHook(new Thread(Project3::end));
        
        System.out.println("Initializing");
        init(false);
        System.out.println("Initialization done. Finding pillar.");
        detectPillar();
        System.out.println("Found pillar. Turning to face the wall.");
        faceWall();
        System.out.println("Facing the wall. Going to the wall.");
        goToWall();
        System.out.println("At the wall. Following the wall to the destination.");
        followWall(TARGET_DISTANCE);
    }
    ...
    static void detectPillar() throws IOException, InterruptedException {
        int dist = 0;
        while (true) {
            collectSensors();
            if (getBump()) break;
    ...
    static void faceWall() throws IOException, InterruptedException {
        sendSpinLeft(100);
        while (true) {
            collectSensors();
            if (getWall()) break;
    ...
    static void goToWall() throws IOException, InterruptedException {
        sendForward(SPEED);
        while (true) {
            collectSensors();
            if (getBump()) {
    ...
    static void followWall(int targetDistance) throws IOException, InterruptedException {
        int distanceTravelled = 0;
        
        while(true) {
            collectSensors();
            distanceTravelled += getDistance();
            System.out.printf("Distance Travelled: %d / %d%n", distanceTravelled, targetDistance);
    ...
    static int getDistance() {return getSignedValues(12, 2);}
